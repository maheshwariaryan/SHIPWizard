...
class MLInsuranceQA:
    def __init__(self, data: pd.DataFrame, model_path: str = "insurance_model"):
        self.data = data
    ...
    def get_answer(self, question: str) -> str:
        ...
        try:
            ...
            if pd.notna(service_info['Limitations/Important Notes']) and \
               service_info['Limitations/Important Notes'] != 'None':
                response += f"\n- Important Notes: {service_info['Limitations/Important Notes']}"
...
def main():
    ...
    data = pd.DataFrame({
        'Category of Service': ['Primary Care Visit', 'Specialist Visit', 'Preventive Care/Screening/Immunization',
                              # ... (rest of the data remains the same)
                             ],
        'UHS Cost': ['No charge', 'No charge', 'No charge for Student/Spouse',
                    # ... (rest of the data remains the same)
                    ],
        'In-network Cost': ['10% coinsurance', '10% coinsurance', 'No charge for children',
                           # ... (rest of the data remains the same)
                          ],
        'Out-of-network Cost': ['40% coinsurance', '40% coinsurance', '40% coinsurance',
                               # ... (rest of the data remains the same)
                              ],
        'Limitations/Important Notes': ['None', 'Chiropractic care subject to coinsurance',
                                      # ... (rest of the data remains the same)
                                     ]
    })
